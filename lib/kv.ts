...
const memoryStore: {
  ...
  likes: { jom: 0, ten: 0, jino: 0, pao: 0 },
  userVotes: {},
};
...
export async function getLikes(): Promise<Record<string, number>> {
  try {
    if (process.env.KV_REST_API_URL && process.env.KV_REST_API_TOKEN) {
      ...
      return likes || { jom: 0, ten: 0, jino: 0, pao: 0 };
    }
  } catch (error) {
...
export async function getUserVote(userId: string): Promise<string | null> {
  try {
    if (process.env.KV_REST_API_URL && process.env.KV_REST_API_TOKEN) {
      const userVotes = await kv.get<Record<string, string | null>>('userVotes');
...
export async function setUserVote(userId: string, contestantId: string | null): Promise<void> {
  try {
    if (process.env.KV_REST_API_URL && process.env.KV_REST_API_TOKEN) {
      const userVotes = await kv.get<Record<string, string | null>>('userVotes') || {};
